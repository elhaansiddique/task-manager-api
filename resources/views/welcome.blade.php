                    ...
                    <header class="grid grid-cols-2 items-center gap-2 py-10 lg:grid-cols-3">
                        ...
                        <html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
                            <head>
                                ...
                                <style>
                                    /* Minimal styles for a focused auth page */
                                    html,body{height:100%;margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial}
                                    .center{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f3f4f6;padding:16px}
                                    .card{width:100%;max-width:420px;background:#ffffff;border-radius:8px;padding:24px;box-shadow:0 10px 30px rgba(2,6,23,0.08);margin:auto}

                                    /* Tabs */
                                    .tabs{display:flex;gap:8px;margin-bottom:18px}
                                    .tab{flex:1;padding:10px 12px;text-align:center;border-radius:6px;cursor:pointer;background:#f8fafc;transition:all 0.15s ease;color:#111827;border:1px solid transparent}
                                    .tab:not(.active){background:#f8fafc;color:#111827;border-color:transparent}
                                    .tab.active{background:#111827;color:#fff;font-weight:600;border-color:#111827;box-shadow:0 6px 18px rgba(17,24,39,0.08)}
                                    .tab:focus{outline:none;box-shadow:0 0 0 3px rgba(17,24,39,0.06)}

                                    /* Labels: make register & login labels visually consistent */
                                    label{display:block;font-size:14px;color:#111827;margin-bottom:6px}
                                    input[type=text],input[type=email],input[type=password]{width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:12px;background:#fff;color:#111827}
                                    input[type=text]:focus,input[type=email]:focus,input[type=password]:focus{outline:none;border-color:#111827;box-shadow:0 0 0 3px rgba(17,24,39,0.06)}

                                    /* Ensure register form elements follow same visual style as login */
                                    #form-register label, #form-login label{color:#111827}
                                    #form-register input, #form-login input{border-color:#d1d5db}

                                    button{width:100%;padding:10px;border-radius:6px;border:0;background:#111827;color:#fff;font-weight:600}
                            ...
                            <body>
                                <div class="center">
                                    <div class="card" style="">
                                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px; flex-direction: column; gap: 10px;">
                                            <h2 style="margin:0;font-size:20px;color:#111827;font-weight:600">Welcome</h2>
                                            <div style="font-size:14px;color:#6b7280">Task Delegation System</div>
                                        </div>
                                        ...
                                        {{-- Display validation errors (first errors) --}}
                                        @if ($errors->any())
                                        ...
                                        <form id="form-login" method="POST" action="{{ url('/login') }}">
                                            @csrf
                                            <label for="login-email" style="font-size:14px;color:#111827">Email</label>
                                            <input id="login-email" name="email" type="email" value="{{ old('email') }}" required />

                                            <label for="login-password" style="font-size:14px;color:#111827">Password</label>
                                            <input id="login-password" name="password" type="password" required />

                                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
                                                <label style="font-size:13px;color:#6b7280"><input type="checkbox" name="remember"> Remember me</label>
                                                <a href="#" class="small-link">Forgot?</a>
                                            </div>
                                        ...
                                        <form id="form-register" method="POST" action="{{ url('/register') }}" style="display:none;margin-top:6px">
                                            @csrf
                                            <label for="reg-name" style="font-size:14px;color:#111827;margin-bottom:6px">Full name</label>
                                            <input id="reg-name" name="name" type="text" value="{{ old('name') }}" required style="width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:16px;background:#fff;color:#111827" />

                                            <label for="reg-email" style="font-size:14px;color:#111827;margin-bottom:6px">Email</label>
                                            <input id="reg-email" name="email" type="email" value="{{ old('email') }}" required style="width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:16px;background:#fff;color:#111827" />

                                            <label for="reg-password" style="font-size:14px;color:#111827;margin-bottom:6px">Password</label>
                                            <input id="reg-password" name="password" type="password" required style="width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:16px;background:#fff;color:#111827" />

                                            <label for="reg-password-confirm" style="font-size:14px;color:#111827;margin-bottom:6px">Confirm password</label>
                                            <input id="reg-password-confirm" name="password_confirmation" type="password" required style="width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:16px;background:#fff;color:#111827" />

                                            <label for="reg-role" style="font-size:14px;color:#111827;margin-bottom:6px">Role</label>
                                            <select id="reg-role" name="role" required style="width:100%;padding:10px;border-radius:6px;border:1px solid #d1d5db;margin-bottom:16px;background:#fff;color:#111827;appearance:none;background-image:url('data:image/svg+xml;charset=US-ASCII,<svg width=\"24\" height=\"24\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\" fill=\"%23111827\"/></svg>');background-repeat:no-repeat;background-position:right 10px center">
                                                <option value="employee">Employee</option>
                                                <option value="manager">Manager</option>
                                            </select>

                                            <button type="submit" style="width:100%;padding:12px;border-radius:6px;border:0;background:#111827;color:#fff;font-weight:600;cursor:pointer;transition:all 0.15s ease;margin-top:8px">Create account</button>
                                        </form>

                                        <div class="muted">By continuing you accept the <a href="#" class="small-link">terms</a>.</div>
                                    </div>
                                </div>
                                ...
                                <script>
                                    ...
                                    // Optional: Send login as AJAX if you want JSON (helps client-side apps expecting JSON)
                                    formLogin.addEventListener('submit', function (e) {
                                        ...
                                        const data = {
                                            ...
                                            remember: document.querySelector('#form-login [name=remember]')?.checked ? 'on' : ''
                                        };
                                        ...
                                        fetch('{{ url('/login') }}', {
                                          ...
                                          .then(json => {
                                              ...
                                              // fallback to full form submit if JSON login fails
                                              console.error(err);
                                              e.target.submit();
                                          });
                                    });
